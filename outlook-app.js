(function(){
  ...
  var HTML = ''
    ...
    +   '<div class="row" id="actions" style="display:none"><button class="btn sec" id="copyLink" style="margin-top:0">Copy link</button><button class="btn green" id="insertLink" style="margin-top:0">Insert into email</button></div>'
  ...
  function renderSlots(){
    $("actions").style.display = SLOTS.length ? "flex" : "none";
  ...
  $("copyLink").onclick = function(){ var msg = $("msg"), sel = selected(); if (!sel.length){ msg.className = "msg err"; msg.textContent = "Tick at least one slot first."; return; } msg.className = "msg"; msg.textContent = "Creating link…"; createLink(sel).then(function(url){ try { navigator.clipboard.writeText(url); } catch(e){} msg.className = "msg ok"; msg.textContent = "✓ Link copied — paste it anywhere."; }).catch(linkErr(msg)); };
  $("insertLink").onclick = function(){ var msg = $("msg"), sel = selected(); if (!sel.length){ msg.className = "msg err"; msg.textContent = "Tick at least one slot first."; return; } msg.className = "msg"; msg.textContent = "Creating link…"; createLink(sel).then(function(url){ insertHtml('<a href="' + url + '">Book a time with me</a>', msg, "✓ Booking link added to your email."); }).catch(linkErr(msg)); };
